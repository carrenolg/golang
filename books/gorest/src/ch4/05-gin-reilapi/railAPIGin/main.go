...
import (
	...
	"carrenolg.io/books/gorest/src/ch4/03-rail-api/dbutils"
...
func GetStation(c *gin.Context) {
	...
	sql := `SELECT ID, NAME, CAST(OPENING_TIME AS CHAR), CAST(CLOSING_TIME AS CHAR) FROM station WHERE id=?`
	err := DB.QueryRow(sql, id).Scan(&station.ID, &station.Name, &station.OpeningTime, &station.ClosingTime)
	// Show db error
	if err != nil {
		log.Println(err)
		c.JSON(500, gin.H{
...
func CreateStation(c *gin.Context) {
	...
	if err := c.BindJSON(&station); err == nil {
		...
		sql := `INSERT INTO station (NAME, OPENING_TIME, CLOSING_TIME) VALUES(?, ?, ?)`
...
// RemoveStation handles the POST
...
func main() {
	...
	r.Run(":8000") // Default listen and server on 0.0.0.0:8080
}
